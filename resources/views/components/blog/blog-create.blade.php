<x-layout color="black">
    <x-header>Įkelkite naują blogą</x-header>
    <form method="POST" action="/blog/store" enctype="multipart/form-data" class="flex justify-center  ">
        @csrf
        ...
        <div class="">
            ...
            <input class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-gray-800 placeholder-gray-500" type="file" name="image_path[]" multiple>
            @error('image_path')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
